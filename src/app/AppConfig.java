...
public class AppConfig {
	...
	public static void readConfig(String configName, int serventId){
		...
		try {
			WEAK = Integer.parseInt(properties.getProperty("weak"));
		} catch (NumberFormatException e) {
			timestampedErrorPrint("Problem reading weak. Exiting...");
			System.exit(0);
		}

		try {
			STRONG = Integer.parseInt(properties.getProperty("strong"));
		} catch (NumberFormatException e) {
			timestampedErrorPrint("Problem reading servent_count. Exiting...");
			System.exit(0);
		}
		...
		if (ROOT_DIR == null) {
			...
			ROOT_DIR = properties.getProperty("root") + "/servent"+serventId;
		}
...
//		}
	...
	public static int sha1HashToChordKey(String input) {
		try {
			MessageDigest messageDigest = MessageDigest.getInstance("SHA-1");
			byte[] hashBytes = messageDigest.digest(input.getBytes(StandardCharsets.UTF_8));
			BigInteger hashInteger = new BigInteger(1, hashBytes);
			return hashInteger.mod(BigInteger.valueOf(AppConfig.chordState.CHORD_SIZE)).intValue();
