...
public class MultipleServentStarter {
	...
	private static void startServentTest(String testName) {
		...
		ProcessBuilder bsBuilder = new ProcessBuilder("java", "-cp", "out/production/projekat-spetrovic4520rn", "app.BootstrapServer", String.valueOf(AppConfig.BOOTSTRAP_PORT));
		try {
		...
		for(int i = 0; i < serventCount; i++) {
			try {
				ProcessBuilder builder = new ProcessBuilder("java", "-cp", "out/production/projekat-spetrovic4520rn", "app.ServentMain",
